	...
	if(!empty($help)) {
		...
		echo $this->element('help', [
			...
			'options' => [
				...
				'icon'        => ['false', 'An icon to include on the button (e.g. <code>"pencil"</code>). Will be pulled to right, to center put icon in title instead.'],
...
<?php if($type == 'link'): ?>
	<a class="btn btn-<?= $class ?>" href="<?= $url ?>" role="button" title="<?= $tooltip ?>" <?php if($confirm) echo 'onclick="return confirm(\''. $confirm .'\')"' ?> <?= $vars ?>><?= $title ?><?php if($icon) echo $this->element('SiteManager.Bootstrap/icon', ['icon' => $icon .' pull-right']) ?></a>
<?php else: ?>
	<button class="btn btn-<?= $class ?>" type="<?= $type ?>" onclick="<?php if($confirm){ echo 'if(confirm(\''. $confirm .'\')) '. $url; } else { echo $url; } ?>" title="<?= $tooltip ?>" <?= $vars ?>><?= $title ?><?php if($icon) echo $this->element('SiteManager.Bootstrap/icon', ['icon' => $icon .' pull-right']) ?></button>
<?php endif; ?>
